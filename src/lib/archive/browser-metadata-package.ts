...
export function buildBrowserMetadataPackage(input: {
  ...
  const incompleteNote = isR2
    ? [
        ...
        "Included: metadata.json, states.json, notes.md" +
          ...
          ".",
        "Portable HTML mint-package is available on demand from the published revision (admin Download HTML Package).",
  ...
  if (isR2) {
    // R2/D1 path: portable HTML mint-package is downloaded on demand from the
    // published revision; do not warn about omitted perception.html here.
  } else {
    warnings.push(
      "Local browser package omits perception.html and manifest.json. Use Export mint package after a full local regenerate, or Download HTML Package after publish on D1.",
    );
  }
